...
class TemperatureMapLayer(MapLayer):
    def __init__(self, coordinates=None, color=[0, 0, 1, 1], width=2, **kwargs):
        ...
        self._line_points_offset = ((0, 0), [0, 0, 0, 1])
        self.zoom = 0
        self.lon = 0
        self.lat = 0
        self.ms = 0
    ...
    def add_point(self, point, color):
        if self._coordinates is None:
            ...
            self._coordinates = []
        self._coordinates.append((point, color))
    ...
    def calc_line_points(self):
        ...
        self._line_points_offset = (self.get_x(self.coordinates[0][0][1]),
                                    self.get_y(self.coordinates[0][0][0]))
        # Since lat is not a linear transform we must compute manually
        self._line_points = [((self.get_x(lon) - self._line_points_offset[0],
                               self.get_y(lat) - self._line_points_offset[1]), color)
                               for (lat, lon), color in self.coordinates]
    ...
    def _draw_line(self, *args):
        ...
        with self.canvas:
            ...
            # Color(*self.color)
            Line(points=list(map(lambda point: point[0], self.line_points)), width=self._width)
            for i in range(len(self.line_points) - 1):
                lat1, lon1 = self.line_points[i][0]
                lat2, lon2 = self.line_points[i + 1][0]
                Color(*self.line_points[i + 1][1])
                Line(points=[lat1, lon1, lat2, lon2], width=10)
